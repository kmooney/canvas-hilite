...
function CodeCanvas(canvas) {
	...
	this.viewport.right = Math.floor((canvas.width-this.gutter_size)/this.context.measureText('m').width);
	var can = this;
...
CodeCanvas.prototype.render_text = function() {
	// replay the render state up to the initial viewport.
	var viewport_start = this.viewport.start;
	for (var i = 0; i < viewport_start; i++) {
		if (typeof this.lines[i] !== "undefined") {
			var line_char_count = this.lines[i].length;
			for (var j = 0; j < line_char_count; j++) {
				if (this.update_render_state(this.lines[i][j]) !== false) {
					this.context.fillStyle = this.render_state.font_color;
				}
			}
		}
	}
	// now start rendering text, and updating render state simultaneously.
