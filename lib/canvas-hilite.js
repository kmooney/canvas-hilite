function CodeCanvas(canvas) {
    ...
    this.line_height = font_size * 1.5
    
    // configure the viewport pased on the canvas height
    
    this.viewport.start = 0;
    this.viewport.end = Math.floor(canvas.width/(font_size * 2))
        ...
        if (typeof this.lines[i] != "undefined") {
            for (var j = 0; j < this.lines[i].length; j++) {
                if (typeof this.lines[i][j] != "undefined") {
                    var coords = [x_coord + (this.emSize.width*j), Math.floor(y_coord + this.font_size + this.font_size / 2)]
...
CodeCanvas.prototype.render = function() {
    ...
    this.context.clearRect(0, 0, this.context.canvas.width, font_size * 1.5 * (this.viewport.end+1));
    this.render_text();
    this.draw_cursor();
