...
function CodeCanvas(canvas) {
    ...
    this.viewport.end = Math.floor(canvas.width/(render_state.font_size * 2))
...
CodeCanvas.prototype.render_text = function() {
    for (var i = this.viewport.start; i < this.viewport.end; i++) {
        var y_coord = (i - this.viewport.start) * render_state.font_size * 1.5
        var x_coord = this.gutter_size;
        if (typeof this.lines[i] != "undefined") {
            for (var j = 0; j < this.lines[i].length; j++) {
                if (typeof this.lines[i][j] != "undefined") {
                    var updated = this.update_render_state(this.lines[i][j])
                    var coords = [x_coord + (this.emSize.width*j), Math.floor(y_coord + render_state.font_size + render_state.font_size / 2)]
...
CodeCanvas.prototype.draw_cursor = function() {
    ...
    this.context.fillRect(
        ...
        coords[1] * (render_state.font_size * 1.5),
        1, 
        render_state.font_size * 1.5 + (render_state.font_size / 4)
    )
}

CodeCanvas.prototype.render = function() {
    //console.log([this.viewport.start, this.viewport.end])
...
CodeCanvas.prototype.reset_state = function() {
    this.render_state = RenderState.copy(this.start_state);
    this.render_states = {}
    for (s in this.initial_states) {
...
CodeCanvas.prototype.update_render_state = function(match_code) {
    if (typeof this.render_states[match_code] != "undefined") {
        ...
        if (this.render_states[match_code].toggle) {
            this.render_states[match_code].state = RenderState.copy(current_state)
        }
        return true;
    }
    return false;
